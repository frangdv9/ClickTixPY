...
class Turno_Controller:
    ...
    def traerHorarioPorId(self, id):
        try:
            self.conexion.cursor.execute("SELECT horario FROM TURNO where id=?", (id,))
    ...
    def ingresar_turno(self, horario):
        try:
            self.conexion.cursor.execute("INSERT INTO TURNO (horario) VALUES (?)", (horario,))
            self.conexion.conexion.commit()
            return True
        except sqlite3.Error as e:
    ...
    def modificar_turno(self, id, horario):
        try:
            self.conexion.cursor.execute("UPDATE TURNO SET horario=? WHERE id=?", (horario, id))
            self.conexion.conexion.commit()
            return True
        except sqlite3.Error as e:
        ...
        while opcion_turno != "5":
            ...
            if opcion_turno == "1":
                ...
                if self.ingresar_turno(horario):
                    print("Turno ingresado con exito.")
                else:
